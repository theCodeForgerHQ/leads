...
# --------------------------------------------------------------------------
_KEY_PATTERNS = [
    re.compile(r'window\.AlgoliaOpts\s*=\s*\{[^}]*"key"\s*:\s*"([A-Za-z0-9+/=]{20,})"', re.I),
...
_WAAS_COMPANY_BASE = "https://www.workatastartup.com/companies"


class CompanyPageJobsSource:
    """Fetch open jobs by scraping individual WaaS company pages.

    This is the primary job source. It fetches the server-rendered HTML for
    each eligible company, extracts jobs from the embedded React props, and
    returns them as Job objects for the classifier to evaluate.

    Only companies with isHiring=True in the yc-oss mirror are fetched,
    which keeps the request count manageable while accepting the small risk
    that some companies have stale isHiring flags.
    """
    ...
    def software_jobs(self, companies: dict[int, Company]) -> list[Job]:
        hiring = {cid: c for cid, c in companies.items() if c.is_hiring}
        log.info("Fetching company pages for %d hiring companies ...", len(hiring))
        jobs: list[Job] = []
        for company_id, company in hiring.items():
            page_jobs = self._fetch_company_jobs(company)
            jobs.extend(page_jobs)
        log.info("Open jobs fetched from company pages: %d", len(jobs))
        return jobs

    def _fetch_company_jobs(self, company: Company) -> list[Job]:
        url = f"{_WAAS_COMPANY_BASE}/{company.slug}"
        try:
        ...
        if not html:
            return []
        return self._parse_jobs(html, company)

    def _parse_jobs(self, html: str, company: Company) -> list[Job]:
        m = _DATA_PAGE_RE.search(html)
        if not m:
            return []
        try:
            props = json.loads(_html.unescape(m.group(1))).get("props", {})
            raw_jobs = props.get("company", {}).get("jobs", [])
        ...
        for rj in raw_jobs:
            ...
            if not job_id or not title:
                continue
            locs = [loc.strip() for loc in (rj.get("location") or "").split("/") if loc.strip()]
            result.append(Job(
                object_id=str(job_id),
                company_id=company.id,
                title=title,
                role=None,
                job_type=rj.get("jobType"),
